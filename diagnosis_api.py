...
models = {
    'Random Forest': joblib.load('aimodels/Random Forest.joblib'),
    'Decision Tree': joblib.load('aimodels/Decision Tree.joblib'),
    'Logistic Regression': joblib.load('aimodels/Logistic Regression.joblib'),
    'Support Vector Machine': joblib.load('aimodels/Support Vector Machine.joblib'),
    'Gaussian Naive Bayes': joblib.load('aimodels/Gaussian Naive Bayes.joblib')
}
label_encoder = joblib.load('aimodels/label_encoder.joblib')
scaler = joblib.load('aimodels/scaler.joblib')
...
def encode_symptoms(symptoms):
    return np.array([1 if symptom in symptoms else 0 for symptom in all_symptoms])

@app.post("/diagnose")
async def diagnose(data: SymptomRequest):
    if not data.symptoms:
        raise HTTPException(status_code=400, detail="No symptoms provided")
    
    encoded_vector = encode_symptoms(data.symptoms)
    scaled_vector = scaler.transform(encoded_vector.reshape(1, -1))
    ...
    probabilities = {k: v / total * 100 for k, v in prediction_counts.items()}
